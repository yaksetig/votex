...
export const deleteElectionFromDb = async (electionId: string) => {
  try {
    // Use Supabase's built-in transaction capabilities if available
    // Or we can implement a manual approach
    ...
    if (votesData && votesData.length > 0) {
      console.log(`Found ${votesData.length} votes to delete for election ${electionId}`);
      ...
      console.log(`Successfully deleted ${votesData.length} votes`);
      
      // Add a small delay to ensure the votes deletion has propagated
